...
+ (NSDictionary *)refreshVersionData:(NSString *)filePath {
    if(filePath==nil) {
        NSLog(@"ERROR No apps information was extracted from system profiler");
        return nil;
    }
    
    NSArray<NSDictionary*> *output = [NSArray arrayWithContentsOfFile:filePath];
    if(output==nil) {
        NSLog(@"ERROR Could not parse %@ as a plist file.", filePath);
        return nil;
    }
    ...
    for (NSDictionary* appInfo in apps) {
        ...
        if([appName rangeOfString:@"Adobe Premiere Pro"].location != NSNotFound) {
            ...
            if(appVersion!=nil) {   //appVersion==nil is a valid outcome on some apps
